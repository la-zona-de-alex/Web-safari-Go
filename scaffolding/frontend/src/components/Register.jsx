...
export default function Register() {
  ...
  function googleSignIn() {
    ...
    const popup = window.open(API_BASE + '/api/auth/google', 'GoogleSignUp', `width=${width},height=${height},left=${left},top=${top}`);
    ...
    const checkPopup = setInterval(()=>{ if (popup && popup.closed) clearInterval(checkPopup) }, 500);
  }

  async function submit(e) {
    e.preventDefault()
    if (!firstName || !lastName || !email || !password) return setMsg('Completa todos los campos')
    ...
    try {
      ...
      // Si es conductor, bloqueamos el acceso hasta verificación manual
      if (role === 'driver') {
        setMsg('Tus credenciales están siendo verificadas. En unos momentos serás atendido para formar parte de Safari Go')
        // keep user on the page; do not redirect
      } else {
        // Autologin: intentar iniciar sesión automáticamente para mostrar el botón de acción
        try {
          const loginRes = await fetch(`${API_BASE}/api/login`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email, password }) })
          const loginData = await loginRes.json()
          if (loginRes.ok) {
            localStorage.setItem('token', loginData.token)
            localStorage.setItem('user', JSON.stringify(loginData.user))
            setMsg('Registrado y autenticado. ¡Bienvenido!')
            window.location.hash = '#/'
            return
          }
        } catch (e) {
          // fallthrough to ask login
        }
        setMsg('Registrado. Por favor inicie sesión.')
      ...
      <form onSubmit={submit} encType="multipart/form-data">
        ...
        {role === 'driver' && (
          <>
            ...
            <input name="vehicle_photo" type="file" onChange={onFileChange} required />
            <label>Foto de la licencia</label>
            <input name="license_photo" type="file" onChange={onFileChange} required />
            <label>Foto del conductor</label>
            <input name="driver_photo" type="file" onChange={onFileChange} required />
            <label>Foto del DNI</label>
            <input name="dni_photo" type="file" onChange={onFileChange} required />
          </>
        )}
        {role === 'passenger' && (
          <>
            <label>Foto (pasajero)</label>
            <input name="passenger_photo" type="file" accept="image/*" onChange={onFileChange} />
          </>
        )}
